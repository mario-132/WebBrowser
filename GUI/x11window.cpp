...
void X11Window::createWindow(std::string title, int width, int height, int framebufferWidth, int framebufferHeight)
{
    ...
    displayBuffer = (unsigned char*)malloc(framebufferWidth*framebufferHeight*4);
    ...
    ximage = XCreateImage(display, visual, 24, ZPixmap, 0,
                          (char*)displayBuffer, framebufferWidth, framebufferHeight, 32, 0);
}

void X11Window::drawString(std::string str, int x, int y)
{
    XDrawString(display, window, DefaultGC(display, 0), x, y, str.c_str(), str.size());
}
